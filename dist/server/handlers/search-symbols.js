...
const normalizePlatform = (value) => value.toLowerCase().replaceAll(/[^a-z\d]+/g, '');
export const buildSearchSymbolsHandler = (context) => {
    ...
    return async (args) => {
        ...
        if (platform) {
            ...
            filteredResults = filteredResults.filter(result => result.platforms.some(p => normalizePlatform(p).includes(platformNormalized)));
        }
        if (symbolType) {
        ...
        if (useGlobal) {
            lines.push('✅ **Comprehensive Index:** Global cached symbol database is available.', '');
        }
        ...
        if (filteredResults.length > 0) {
            ...
            lines.push(`No symbols matched those terms within this ${useGlobal ? 'scope' : 'technology'}.`, '', '**Search Tips:**', '• Try wildcards: `Grid*` or `*Item`', '• Use broader keywords: "grid" instead of "griditem"', '• Check spelling and try synonyms', '');
            if (isSpecificSymbol) {
                ...
                try {
                    ...
                    lines.push('**✅ Found exact symbol via live lookup.**', '', `### ${fallbackEntry.title}`, `   • **Kind:** ${fallbackEntry.kind}`, `   • **Path:** ${fallbackEntry.path}`, `   • **Platforms:** ${fallbackEntry.platforms.length > 0 ? fallbackEntry.platforms.join(', ') : 'All platforms'}`, `   ${fallbackEntry.abstract}`, '');
                }
                catch (error) {
